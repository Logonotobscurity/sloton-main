...
import { genaiArticles, genaiSections } from "@/lib/data/genai-articles";

export const metadata = generateMetadata({
  title: "Generative AI — strategy, use cases, safety & governance",
  description:
    "LOG_ON cluster on GenAI for Nigerian, African, and global operators. Definitions, risks, models, tools. 90-day review.",
  canonical: "/insights/generative-ai",
});

export default function GenerativeAiClusterPage() {
  const pick = genaiArticles.find((a) => a.slug === "what-is-generative-ai-nigeria-2026")
    || genaiArticles.find((a) => a.slug === "what-is-generative-ai");
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-[1100px] mx-auto">
        ...
        <h1 className="mt-4 text-4xl md:text-5xl font-headline">Generative AI</h1>
        <p className="mt-4 text-lg text-muted-foreground max-w-3xl">
          GenAI strategy, use cases, safety, and governance — operator-authored by LOG_ON (Lagos).
        </p>
        ...
        <dl className="mt-6 grid grid-cols-3 gap-3 max-w-lg">
          <div className="rounded-xl border border-border p-3">
            ...
            <dd className="text-2xl font-headline text-primary">{genaiArticles.length}</dd>
          </div>
          <div className="rounded-xl border border-border p-3">
        ...
        <div className="mt-14 space-y-12">
          {genaiSections.map((section) => {
            const items = genaiArticles.filter((a) => a.section === section);
